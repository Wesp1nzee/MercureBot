...
async def message_with_text(callback: CallbackQuery, state: FSMContext):
    if callback.data == "physics":
        await callback.message.edit_text(
            "Выбери тип задачи:", reply_markup=await ikb.create_kb_task_physics()
        )
    else:
...
@router.callback_query(
    FactoryTask.filter(F.object == "physics" and F.task_number == "18"),
    StateMachine.task_selection_physics,
)
async def message_with_text(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
...
@router.callback_query(
    FactoryTask.filter(F.object == "physics"), StateMachine.task_selection_physics
)
@router.callback_query(
    FactoryTask.filter(F.object == "physics"), StateMachine.error_message
)
@router.callback_query(
    FactoryTask.filter(F.object == "physics"), StateMachine.task_18_selection_physics
)
async def message_with_text(
    ...
    try:
        ...
        await callback.answer(text="Этот раздел ещё не готов:0", show_alert=True)


@router.callback_query(
    FactoryTask.filter(F.object == "physics" and F.direction == "Next"),
    StateMachine.leaf_task_physics,
)
async def message_with_text(
    callback: CallbackQuery, state: FSMContext, callback_data: FactoryTask
):
    ...
    if task_count + 1 <= await container_phy.get_item(task_number):
        await db.update_user_task(
            callback.from_user.id, "physics", task_number, sign="+"
        )
...
@router.callback_query(
    FactoryTask.filter(F.object == "physics" and F.direction == "Back"),
    StateMachine.leaf_task_physics,
)
async def message_with_text(
    callback: CallbackQuery, state: FSMContext, callback_data: FactoryTask
...
@router.callback_query(
    FactoryTask.filter(F.object == "physics" and F.decision == True),
    StateMachine.leaf_task_physics,
)
async def message_with_text(
    callback: CallbackQuery, state: FSMContext, callback_data: FactoryTask
    ...
    task_number = user_data["task_number"]
    task_count = callback_data.task_count
    answer = await db.get_task_decision(
        user_id=task_count, task_number=task_number, object="physics"
    )
    if answer:
        await callback.answer(text=f"Ответ: {answer}", show_alert=True)
    else:
        await callback.answer(
            text="Извините, но у нас нет ответа на эту задачку:((", show_alert=True
        )
